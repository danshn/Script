...
!(async () => {
  ...
  let res = await getAuthorShareCode('https://raw.githubusercontent.com/msechen/shareCodes/main/pigPet.json')
  if (!res) {
  $.http.get({url: 'https://raw.githubusercontent.com/msechen/shareCodes/main/pigPet.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
  await $.wait(1000)
  res = await getAuthorShareCode('https://raw.githubusercontent.com/msechen/shareCodes/main/pigPet.json')
  }
  $.shareCodes = [...(res || [])]
  ...
  for (let j = 0; j < cookiesArr.length; j++) {
    ...
    if ($.canHelp && ($.shareCodes && $.shareCodes.length) ) {
      for (let item of $.shareCodes) {
        console.log(`【京东账号${$.index}】${$.UserName}去助力${item}`)
